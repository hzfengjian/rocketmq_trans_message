...
import org.apache.rocketmq.store.transaction.TransactionStateFileService;
...
public class MessageStoreConfig {
    ...
    private int tranStateTableMapedFileSize = 2000000 * TransactionStateFileService.TSS_STORE_UNIT_SIZE;


    //----------------------------- 分布式事务相关配置----------------------------------//
    // 事务回查至少间隔时间
    private long checkTransactionMessageAtleastInterval = 1000 * 3 * 60;

    // 事务回查定时间隔时间
    private long checkTransactionMessageTimerInterval = 1000 * 30;

    //事务使用数据库相关信息
    @ImportantField
    private boolean transactionLogInDB = true;
    ...
    private long flushDelayTransOffsetInterval = 1000 * 5;
    @ImportantField
    ...
    public boolean isTransactionLogInDB() {
        return transactionLogInDB;
    }

    public void setTransactionLogInDB(boolean transactionLogInDB) {
        this.transactionLogInDB = transactionLogInDB;
    }
    ...
    public long getFlushDelayTransOffsetInterval() {
        return flushDelayTransOffsetInterval;
    }

    public void setFlushDelayTransOffsetInterval(long flushDelayTransOffsetInterval) {
        this.flushDelayTransOffsetInterval = flushDelayTransOffsetInterval;
    }
