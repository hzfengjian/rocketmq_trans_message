...
public class MappedFileQueue {
    ...
    public int deleteExpiredFileByTime(final long expiredTime,
        ...
        if (null != mfs) {
            for (int i = 0; i < mfsLength; i++) {
                MappedFile mappedFile = (MappedFile) mfs[i];
                if (minTransOffset == 0 || mappedFile.getFileFromOffset() + mappedFile.fileSize < minTransOffset) {
    ...
    public List<MappedFile> findGreaterThanMappedFileByOffset(final long offset) {
        List<MappedFile> subList = new ArrayList<MappedFile>();
        try {
