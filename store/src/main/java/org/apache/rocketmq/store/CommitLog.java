...
import static org.apache.rocketmq.common.message.MessageConst.MSG_ORIGION_QUEUE_OFFSET;
...
public class CommitLog {
    ...
    public int deleteExpiredFile(//
                                 ...
                                 final long minTransOffset
    ) {
        return this.mappedFileQueue.deleteExpiredFileByTime(expiredTime, deleteFilesInterval, intervalForcibly, cleanImmediately, minTransOffset);
    }
    ...
    public DispatchRequest checkMessageAndReturnSize(java.nio.ByteBuffer byteBuffer, final boolean checkCRC, final boolean readBody) {
        try {
            ...
            String keys = "";
            String uniqKey = null;
            String produceGroup = null;
            String orignQueueOffest = null;
            ...
            if (propertiesLength > 0) {
                ...
                produceGroup = propertiesMap.get(MessageConst.PROPERTY_PRODUCER_GROUP);
            ...
            return new DispatchRequest(//
                    ...
                    orignQueueOffest != null ? Long.parseLong(orignQueueOffest) : queueOffset,
                    produceGroup,
                    checkImmunityTimeOutTimestamp
            );
        } catch (Exception e) {
        }
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        if (magicCode != MESSAGE_MAGIC_CODE) {
            log.info("isMappedFileMatchedRecover :mappedFile {} : magicCode!=MESSAGE_MAGIC_CODE", mappedFile.getFileName());
            return false;
        }

        long storeTimestamp = byteBuffer.getLong(MessageDecoder.MESSAGE_STORE_TIMESTAMP_POSTION);
        if (0 == storeTimestamp) {
            log.info("isMappedFileMatchedRecover :mappedFile {} : storeTimestamp==0", mappedFile.getFileName());
            return false;
        }

        //事务的优先级要超过其他的checkpoint
        if (defaultMessageStore.getMessageStoreConfig().isTransactionLogInDB()) {
            AtomicLong tranStateTableOffset = defaultMessageStore.getTransactionStateService().getTranStateTableOffset();
            long transactionOffset = tranStateTableOffset.get();
        ...
        log.info("isMappedFileMatchedRecover default");
        return false;
    }
...
//            }
            ...
            switch (tranType) {
                case MessageSysFlag.TRANSACTION_PREPARED_TYPE:
                    if (!defaultMessageStore.getMessageStoreConfig().isTransactionLogInDB()) {
                        queueOffset = CommitLog.this.defaultMessageStore.getTransactionStateService().getTranStateTableOffset().get();
                    } else {
                        queueOffset = 0L;
                    }
                    break;
            ...
            //保存之前的MSG_QUERY_OFFSET add by fengjian
            if (!defaultMessageStore.getMessageStoreConfig().isTransactionLogInDB()) {
                Map<String, String> propertiesMap = MessageDecoder.string2messageProperties(msgInner.getPropertiesString());
                propertiesMap.put(MSG_ORIGION_QUEUE_OFFSET, String.valueOf(msgInner.getQueueOffset()));
                msgInner.setPropertiesString(MessageDecoder.messageProperties2String(propertiesMap));
            }
...
//            }
            ...
            switch (tranType) {
                case MessageSysFlag.TRANSACTION_PREPARED_TYPE:
                    if (!defaultMessageStore.getMessageStoreConfig().isTransactionLogInDB()) {
                        CommitLog.this.defaultMessageStore.getTransactionStateService().getTranStateTableOffset().incrementAndGet();
                    }
                    break;
                case MessageSysFlag.TRANSACTION_ROLLBACK_TYPE:
                    break;
